...
namespace TheTechIdea.Beep.Cloud
{
    ...
    public class AzureCloudDocumentDataSource : IDataSource
    {
        ...
        public AzureCloudDocumentDataSource(string datasourcename, IDMLogger logger, IDMEEditor pDMEEditor, DataSourceType databasetype, IErrorsInfo per)
        {
            ...
            if (Dataconnection.ConnectionProp != null)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    DMEEditor?.AddLogMessage("Beep", $"Could not initialize Azure DocumentDB client: {ex.Message}", DateTime.Now, -1, null, Errors.Failed);
                }
            }
            else
            {
        ...
        public ConnectionState Openconnection()
        {
            try
            {
                ...
                if (_cosmosClient != null)
                {
                    ...
                    ConnectionStatus = ConnectionState.Open;
                    DMEEditor?.AddLogMessage("Beep", "Azure DocumentDB connection opened successfully.", DateTime.Now, -1, null, Errors.Ok);
                }
            }
            catch (Exception ex)
            {
                ConnectionStatus = ConnectionState.Broken;
                DMEEditor?.AddLogMessage("Beep", $"Could not open Azure DocumentDB connection - {ex.Message}", DateTime.Now, -1, null, Errors.Failed);
            }
        ...
        public ConnectionState Closeconnection()
        {
            try
            {
                ...
                ConnectionStatus = ConnectionState.Closed;
                DMEEditor?.AddLogMessage("Beep", "Azure DocumentDB connection closed successfully.", DateTime.Now, -1, null, Errors.Ok);
            }
            catch (Exception ex)
            {
                ConnectionStatus = ConnectionState.Broken;
                DMEEditor?.AddLogMessage("Beep", $"Could not close Azure DocumentDB connection - {ex.Message}", DateTime.Now, -1, null, Errors.Failed);
            }
            return ConnectionStatus;
        }
                ...
                if (entitiesToScript != null && entitiesToScript.Count > 0)
                {
                    foreach (var entity in entitiesToScript)
                    {
                        var script = new ETLScriptDet
                        {
                            ...
                            Ddl = $"# Azure DocumentDB container: {entity.EntityName}\n# Use Azure SDK or Portal to create containers"
                        };
                        scripts.Add(script);
                    }
                }
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!disposedValue)
            {
                if (disposing)
                {
                    ...
                    {
                        DMEEditor?.AddLogMessage("Beep", $"Error disposing DocumentDB connection: {ex.Message}", DateTime.Now, -1, null, Errors.Failed);
                    }
                }
                disposedValue = true;
            }
