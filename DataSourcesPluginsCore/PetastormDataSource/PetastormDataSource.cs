...
{
    [AddinAttribute(Category = DatasourceCategory.FILE, DatasourceType = DataSourceType.Petastorm, FileType = "petastorm")]
    public class PetastormDataSource : IDataSource
    {
        ...
        public PetastormDataSource(string datasourcename, IDMLogger logger, IDMEEditor pDMEEditor, DataSourceType databasetype, IErrorsInfo per)
        {
            ...
            {
                _storePath = Path.Combine(Dataconnection.ConnectionProp.FilePath, Dataconnection.ConnectionProp.FileName);
            }
        ...
        public ConnectionState Openconnection()
        {
            try
            {
                if (Directory.Exists(_storePath) || File.Exists(_storePath))
                {
                    ConnectionStatus = ConnectionState.Open;
                    GetEntitesList();
                    DMEEditor?.AddLogMessage("Beep", $"Opened Petastorm store: {_storePath}", DateTime.Now, 0, null, Errors.Ok);
                }
                else
                {
                    ConnectionStatus = ConnectionState.Closed;
                    ErrorObject.Flag = Errors.Failed;
                    ErrorObject.Message = $"Path not found: {_storePath}";
                }
            }
            catch (Exception ex)
            {
                ...
                DMEEditor?.AddLogMessage("Beep", $"Error opening Petastorm store: {ex.Message}", DateTime.Now, 0, null, Errors.Failed);
            }
        ...
        public IEnumerable<string> GetEntitesList()
        {
            ...
            try
            {
                ...
                if (ConnectionStatus == ConnectionState.Open)
                {
                    // Petastorm stores are typically Parquet-based
                    // Look for schema files or parquet files
                    if (Directory.Exists(_storePath))
                    {
                        var parquetFiles = Directory.GetFiles(_storePath, "*.parquet", SearchOption.AllDirectories);
                        foreach (var file in parquetFiles)
                        {
                            var datasetName = Path.GetFileNameWithoutExtension(file);
                    ...
                    else if (File.Exists(_storePath) && _storePath.EndsWith(".parquet"))
                    {
                        datasetNames.Add(Path.GetFileNameWithoutExtension(_storePath));
                    }
        ...
        public IEnumerable<object> GetEntity(string EntityName, List<AppFilter> filter)
        {
            ...
            try
            {
                ...
                if (ConnectionStatus == ConnectionState.Open)
                {
                    // Petastorm datasets are read using Petastorm library
                    // This is a placeholder - actual implementation requires Petastorm library
                    // For now, return basic structure
                    results.Add(new { Name = EntityName, Type = "Petastorm Dataset" });
                }
            }
            catch (Exception ex)
            {
        ...
        {
            ErrorObject.Flag = Errors.Failed;
            ErrorObject.Message = "SQL execution not supported for Petastorm datasets";
            return ErrorObject;
        }
        ...
        {
            ErrorsInfo retval = new ErrorsInfo { Flag = Errors.Failed, Message = "Update operation not supported for Petastorm datasets. Datasets are read-only." };
            return retval;
        }

        public IErrorsInfo DeleteEntity(string EntityName, object DeletedDataRow)
        {
            ErrorsInfo retval = new ErrorsInfo { Flag = Errors.Failed, Message = "Delete operation not supported for Petastorm datasets. Datasets are read-only." };
            return retval;
        }
        ...
        {
            ErrorObject.Flag = Errors.Failed;
            ErrorObject.Message = "Script execution not supported for Petastorm datasets";
            return ErrorObject;
        }
        ...
        {
            ErrorsInfo retval = new ErrorsInfo { Flag = Errors.Failed, Message = "Insert operation not supported for Petastorm datasets. Datasets are read-only." };
            return retval;
        }
        ...
        {
            ErrorsInfo retval = new ErrorsInfo { Flag = Errors.Failed, Message = "Bulk update not supported for Petastorm datasets" };
            return retval;
        }
